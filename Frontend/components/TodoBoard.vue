  ...
  <section id="board" class="py-20 px-4 max-w-6xl mx-auto">
    ...
    >
      <form @submit.prevent="handleAddTodo" class="space-y-6">
        ...
        <div>
          <label class="block text-sm font-semibold mb-2 text-[#1d1d1f]">Your Name</label>
          <input
            type="text"
        ...
        <button type="submit" class="w-full bg-[#1d1d1f] text-white py-4 rounded-xl font-bold hover:bg-black transition-all">
          Post to Stream
        </button>
      </form>
    </div>
...
const handleAddTodo = () => {
  if (!newTodo.value.title.trim() || !newTodo.value.content.trim() || !newTodo.value.author.trim()) return;
  addTodo(newTodo.value);
  newTodo.value = { title: '', content: '', author: '' };
